...
mxArray *process(const mxArray *mximage, const mxArray *mxmask, const mxArray *mxsbin) {
  double *im = (double *)mxGetPr(mximage);
  double *mk = (double *)mxGetPr(mxmask);
  ...
  if (mxGetNumberOfDimensions(mxmask) != 2 ||
      mxGetClassID(mxmask) != mxDOUBLE_CLASS)
    mexErrMsgTxt("Invalid input");
  ...
  for (int x = 1; x < visible[1]-1; x++) {
    for (int y = 1; y < visible[0]-1; y++) {
      // first color channel
	  double *ms = mk + min(x, dims[1]-2)*dims[0] + min(y, dims[0]-2);  
	  if (*ms<0.5 || *(ms+1)<0.5 || *(ms-1)<0.5 || *(ms+dims[0])<0.5 || *(ms-dims[0])<0.5)
	  {
			continue;
	  }
