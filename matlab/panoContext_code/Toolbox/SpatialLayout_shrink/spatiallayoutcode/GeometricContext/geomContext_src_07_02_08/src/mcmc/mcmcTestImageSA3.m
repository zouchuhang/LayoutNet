function [bestpg, bestmap, segs, segpg, pg2, firsten, bestenergy] = ...
...
[pg, segs, segpg] = updateLabProb(vclassifier, hclassifier, segrt, imsegs, spdata, ...
        imdata, smap, (1:nseg), priors, pg, segs, segpg, usedFeatures);
...
energy = getEnergy(pg, imsegs.npixels, smap);
...
for iter = 2:maxiter+1                 
    ...
    for n = 1:maxn
        ...
        if neighbor~=origseg
            ...
            [newpg, segs, segpg] = updateLabProb(vclassifier, hclassifier, segrt, imsegs, spdata, ...
                imdata, smap2, [origseg2 neighbor2], priors, pg, segs, segpg, usedFeatures);           
           
            newenergy = getEnergy(newpg, imsegs.npixels, smap2);

            [bestpg2, bestmap2, bestenergy2, changed] = ...
               updateBest(bestpg, bestmap, newpg, smap2, imsegs.npixels);
    ...
    if mod(iter, 1)==0
        disp(num2str([iter-1 bestenergy vacc(c) hacc(c)])) 
        if DO_DISPLAY
            figure(1), hold off, displayMarginal(bestpg, imsegs.segimage, grayim); 
            figure(4), hold off, displayConfidence(imsegs.segimage, [], bestpg, bestmap)
...
function energy = getEnergy(pg, npixels, smap)
...
function [pg, segs, segpg] = updateLabProb(vclassifier, hclassifier, segrt, imsegs, spdata, imdata, ...
    smap, sind, priors, pg, segs, segpg, usedFeatures)
...
function [bestpg2, bestmap2, bestenergy, changed] = ...
    updateBest(bestpg, bestmap, pg, smap, npixels)
...
bestenergy = getEnergy(bestpg2, npixels, smap);                
