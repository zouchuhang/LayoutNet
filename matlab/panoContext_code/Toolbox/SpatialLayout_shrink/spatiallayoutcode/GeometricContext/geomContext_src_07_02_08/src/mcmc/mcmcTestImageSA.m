function [bestpg, bestmap, energyIter, bestsp] = mcmcTestImageSA(im, imsegs, vclassifierSP, hclassifierSP, ...
...
%smap = mcmcGetGroundTruthSegmentations(imsegs, {adjlist});
%smap = smap{1};
...
segProb = zeros(nsp, 1);
%segProb = updateSegProbSP(imsegs, plabSP, smap, [1:nseg], segProb);
%segProb = updateSegProbGT(imsegs, smap, [1:nseg], segProb);
segProb = updateSegProb(segclassifier, pvSP, phSP, pE, adjlist, imsegs.npixels, smap, 1:nseg, segProb);

pg = zeros(imsegs.nseg, 7);
if DO_LABELS
    pg = updateLabProb(vclassifier, hclassifier, imsegs, spdata, ...
        imdata, smap, (1:nseg), pg, usedFeatures);
end
...
for iter = 2:maxiter+1                 
    ...
    for n = 1:maxn
        ...
        pWhole = 0.25;
        [smap2, newseg, origseg, neighborsegs] = mcmcGenerateProposals2(pE, adjlist, smap, pWhole, 1-max(pg, [], 2).*segProb);
                
        neighborsegs =  [newseg neighborsegs];

        %neighborsegs = [newseg find(segadjmat(newseg, :))];        
        ...
        if neighbor~=origseg
        %disp(num2str([newseg origseg neighbor]))
            if neighbor~=newseg
            ...
            %newSegProb = updateSegProbSP(imsegs, plabSP, smap2, [origseg2 neighbor2], segProb);       
            
            % get geometry likelihood for proposed segmentation
            if DO_LABELS
                newpg = updateLabProb(vclassifier, hclassifier, imsegs, spdata, ...
                     imdata, smap2, [origseg2 neighbor2], pg, usedFeatures);
            end
...
%            newenergy = getEnergy(newSegProb, newpg, imsegs.npixels, smap2);
        ...
        if mod(c, 25)==0        
            ...
            %disp(num2str([max(smap) min(segProb) min(max(pg, [], 2))]))
            pause(0.05);
        end
    ...
    if mod(iter, 1)==0
        ...
        if DO_LABELS
            ...
            %figure(1), hold off, displaySegments(smap, imsegs.segimage, grayim)
        end
...
if ~DO_LABELS
    bestpg = updateLabProb(vclassifier, hclassifier, imsegs, spdata, ...
        imdata, bestmap, (1:max(bestmap)), pg, usedFeatures);    
    figure(1), hold off, displayMarginal(bestpg, imsegs.segimage, grayim); 
    pause(1)
end
...
function [segpg, segps] = sp2segprobs(pg, ps, smap)
nseg = max(smap);
segpg = ones(nseg, size(pg, 2));
segps = ones(nseg, 1);
for k = 1:numel(smap)
    segpg(smap(k), :) = pg(k, :);
    segps(smap(k)) = ps(k);
...
%[segpg, segps] = sp2segprobs(pg, segProb, smap);
%energy = -sum(log(max(segpg, [], 2).*segps));
energy = -log(sum(max(tmppg, [], 2).*segProb(:).*npixels(:))/sum(npixels));
%energy = -log(sum(max(pg, [], 2).*npixels(:))/sum(npixels));
%energy = -log(sum(max(tmppg, [], 2).*npixels)/sum(npixels));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function segProb = updateSegProbSP(imsegs, plabSP, smap, sind, segProb) 
...
for i = 1:nseg
    ...
    maxval = 0.01;
    for k = 1:7
        kval = sum(plabSP(ind, k).*imsegs.npixels(ind))/sum(imsegs.npixels(ind));
        if kval > maxval
            maxval = kval;
        end
    end
...
function segProb = updateSegProbGT(imsegs, smap, sind, segProb)
...
for i = 1:nseg
    ...
    maxval = 0.01;
    for k = 1:7
        kval = sum((imsegs.labels(ind)==k).*imsegs.npixels(ind))/sum(imsegs.npixels(ind));
        if kval > maxval
            maxval = kval;
        end
    end
...
function pg = updateLabProb(vclassifier, hclassifier, imsegs, spdata, imdata, smap, sind, pg, usedFeatures)

nseg = numel(sind);

data = mcmcGetSegmentFeatures(imsegs, spdata, imdata, smap, sind, usedFeatures);

for i = 1:nseg

    s = sind(i);        
    
    vconf = test_boosted_dt_mc(vclassifier, data(i, :));
    vconf = 1 ./ (1+exp(-vconf));
    vconf = vconf / sum(vconf);    
    hconf = test_boosted_dt_mc(hclassifier, data(i, :));
    hconf = 1 ./ (1+exp(-hconf));
    hconf = hconf / sum(hconf);     
    tmpprob = [vconf(1) vconf(2)*hconf vconf(3)];    
    
    spind = find(smap==s);
    pg(spind, :) = repmat(tmpprob, numel(spind), 1);
