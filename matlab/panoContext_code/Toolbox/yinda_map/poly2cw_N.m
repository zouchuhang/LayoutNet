function [x2, y2] = poly2cw_N(x1, y1)
%POLY2CW_N Convert polygon contour to clockwise vertex ordering
%
%   [X2, Y2] = POLY2CW(X1, Y1) arranges the vertices in the polygonal
%   contour (X1, Y1) in clockwise order, returning the result in X2 and Y2.
%   If X1 and Y1 can contain multiple contours, represented either as
%   NaN-separated vectors or as cell arrays, then each contour is converted
%   to clockwise ordering.  X2 and Y2 have the same format (NaN-separated
%   vectors or cell arrays) as X1 and Y1.
%
%   Example
%   -------
%   Convert a counterclockwise-ordered square to clockwise ordering.
%
%       x1 = [0 1 1 0 0];
%       y1 = [0 0 1 1 0];
%       ispolycw(x1, y1)
%       [x2, y2] = poly2cw(x1, y1);
%       ispolycw(x2, y2)
%
%   See also ISPOLYCW, POLY2CCW, POLYBOOL.

% Copyright 2004-2006 The MathWorks, Inc.
% $Revision: 1.1.4.2 $  $Date: 2006/05/24 03:35:20 $

input_is_cell = iscell(x1);
if ~input_is_cell
   % TODO See if inputcheck('xyvector',...) can be modified to allow x and
   % y to be empty, which we need here.
   %    msg = inputcheck('xyvector', x1, y1);
   %    if ~isempty(msg)
   %       error(msg);
   %    end
   input_is_row = (size(x1, 1) == 1);
   [x1, y1] = polysplit_N(x1, y1);
end

% TODO See if inputcheck('cellxyvector',...) can be modified to allow x and
% y to be empty, which we need here.
% msg = inputcheck('cellxyvector', x1, y1);
% if ~isempty(msg)
%    error(msg);
% end

x2 = x1;
y2 = y1;

for k = 1:numel(x1)
   if ~ispolycw_N(x2{k}, y2{k})
      x2{k} = x2{k}(end:-1:1);
      y2{k} = y2{k}(end:-1:1);
   end
end

if ~input_is_cell
   [x2, y2] = polyjoin_N(x2, y2);
   if input_is_row
      x2 = x2';
      y2 = y2';
   end
end
