function [ olines, vp, views, edges, panoEdge, score, angle] ...
            = panoEdgeDetection_mask( img, viewSize, qError, mask )
...
[H,W,~] = size(img);
panoEdge = paintParameterLine(lines, W, H);
panoEdge = panoEdge.*mask;
valid_line_ID = unique(panoEdge(panoEdge>0.5));
lines = lines(valid_line_ID,:);
