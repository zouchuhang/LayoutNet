...
function getBatch(data, sampsize, count, idx)
    ...
    for i = 1, sampsize do
        ...
        data_gt3 = data.gt3[{{idx[count]},{1}, {1,6}}]
        -- data augmentation
        torch.seed() -- randomization
        -- rotate
        torch.seed()
        local r_prob = torch.add(torch.round(torch.mul(torch.rand(1),data.gt3:size(2)-2)), 1)
        if r_prob[1] > 0 then 
        data_inp = torch.cat(data_inp[{{},{},{},{data.inp:size(4) - r_prob[1]+1,data.inp:size(4)}}], data_inp[{{},{},{},{1,data.inp:size(4) - r_prob[1]}}], 4)
        data_gt = torch.cat(data_gt[{{},{},{},{data.inp:size(4) - r_prob[1]+1,data.inp:size(4)}}], data_gt[{{},{},{},{1,data.inp:size(4) - r_prob[1]}}], 4)
        data_gt2 = torch.cat(data_gt2[{{},{},{},{data.inp:size(4) - r_prob[1]+1,data.inp:size(4)}}], data_gt2[{{},{},{},{1,data.inp:size(4) - r_prob[1]}}], 4)
        data_gt3 = data.gt3[{{idx[count]},{r_prob[1]+1}, {1,6}}]
        end
        -- flip
        ...
        if f_prob[1]>0.5 then
            ...
            data_gt3 = data.gt3[{{idx[count]},{r_prob[1]+1}, {7,12}}]
        end
        -- gamma
        torch.seed() 
...
function getValLoss()
    ...
    for i=1, valnumberOfPasses do
        ...
        --print(model.criterion_3:forward(output[3], gt3Mat))
...
function feval(x)
    ...
    for i=1, numberOfPasses do
        ...
        loss_d_3 = torch.mul(loss_d_3, 0.01)
...
for i = 1, iterations do
    ...
    if i % 20 == 0 then
        ...
        if minValLoss > valLoss then
            ...
            torch.save("./model/panofull_joint_box.t7", params_save:double())
            print("------- Model Saved --------")
            params_save = nil
        end
