function [labv, labh] = testImageGraphCuts2(pg, edata, adjlist, eclassifier, ecal, alpha)
...
pEh = pE(vaind);
phSP = phSP(vind, :);
tmplabh = alphaExpansion(phSP, pEh, adjlisth, alpha);   
labh(vind) = tmplabh;
...
function lab = alphaExpansion(plab, pE, adjlist, alpha)

nsp = size(plab, 1);
nadj = size(adjlist, 1);

epenalty = alpha*max(log(pE)-log(1-pE), 0);% -log(1-pE); % 
